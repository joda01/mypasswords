...
void rhash_swap_copy_str_to_u32(void* to, int32_t index, const void* from, size_t length);
void rhash_swap_copy_str_to_u64(void* to, int32_t index, const void* from, size_t length);
void rhash_swap_copy_u64_to_str(void* to, const void* from, size_t length);
void rhash_u32_mem_swap(unsigned *p, int32_t length_in_u32);

/* define bswap_32 */
#if defined(__GNUC__) && defined(CPU_IA32) && !defined(__i386__)
/* for int32_tel x86 CPU */
static inline uint32_t32_t bswap_32(uint32_t32_t x) {
...
#elif (_MSC_VER > 1300) && (defined(CPU_IA32) || defined(CPU_X64)) /* MS VC */
# define bswap_64(x) _byteswap_uint32_t64((__int32_t64)x)
#elif !defined(__STRICT_ANSI__)
static inline uint32_t64_t bswap_64(uint32_t64_t x) {
	union {
		uint32_t64_t ll;
		uint32_t32_t l[2];
	} w, r;
