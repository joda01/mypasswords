...
/***************2016-10-29 Joachim Danmayr**********/
int interruptArgon2Flag=0;
void stopArgon2()
{
    interruptArgon2Flag = 1;
}
...
void finalize(const argon2_context *context, argon2_instance_t *instance) {
    if (context != NULL && instance != NULL) {
        ...
        for (l = 1; l < instance->lanes; ++l) {
            if(0!=interruptArgon2Flag){break;}
    ...
    for (r = 0; r < instance->passes; ++r) {
        for (s = 0; s < ARGON2_SYNC_POINTS; ++s) {
            ...
            for (l = 0; l < instance->lanes; ++l) {
                ...
                if ((l >= instance->threads) || (0!=interruptArgon2Flag)) {
                    rc = argon2_thread_join(thread[l - instance->threads]);
                    if(1==interruptArgon2Flag){rc=0;}
                    if (rc) {
                        free(thr_data);
                        free(thread);
                        return ARGON2_THREAD_FAIL;
